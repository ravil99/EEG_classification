...
from utils_motoric import LSTM_CNN
...
log_folder = f'runs/SCALED_{input_dim}_Channels_FILTERED_{l_freq}_{h_freq}_FRACTION_{start}->{end}_NEW_LSTM_CNN_{layer_dim}_Embedded_Dropout={dropout_rate}_LR={lr}_dataset={dataset}'
...
model = LSTM_CNN(input_size=input_dim, hidden_size=hidden_dim, num_layers=1,
                 num_classes=2)
...
if visualize == True:
    ...
    summary(LSTM_CNN(input_dim, hidden_dim, layer_dim, output_dim),
            input_size=(2, 1000, input_dim), col_names=["kernel_size", "input_size", "output_size", "num_params"],
            verbose=2)
